...
def filter_and_rank_planes(
    ...
    for ac in aircraft_list:
        ...
        if dist_km <= max_distance_km and elevation_deg >= min_elevation_deg:
            ...
            slant_range_km = math.sqrt(dist_km**2 + (ac["altitude_m"] / 1000.0)**2)
            ac_copy["slant_range_km"] = round(slant_range_km, 1)

            # Perceptual Prominence score (3D Slant Range proximity + elevation sightline)
            # Low / close aircraft (low 3D slant range) dominate, boosted by clear overhead sightline
            elev_rad = math.radians(elevation_deg)
            prominence = (100.0 / (max(1.0, slant_range_km) ** 1.2)) * (1.0 + math.sin(elev_rad))
            ac_copy["prominence"] = round(prominence, 2)
            candidates.append(ac_copy)

    candidates.sort(key=lambda x: (-x["prominence"], x["slant_range_km"]))
    return candidates
...
def format_bounce_response(
    ...
    if language == "fr":
        if alt_ft <= 4000:
            elev_desc = "au-dessus" if elevation >= 45 else "bas"
        else:
        ...
        if alt_ft <= 4000:
            elev_desc = "overhead" if elevation >= 45 else "low"
        else:
            elev_desc = "straight up" if elevation >= 45 else ("high" if elevation >= 25 else "far")
        lines = [f"✈️ {airline} ({callsign})"]
        if route:
            lines.append(f"🛫 {route}")
        else:
